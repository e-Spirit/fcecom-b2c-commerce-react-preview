...
import { useAccessToken, useCategory, useCommerceApi, useCustomerType, useCustomerBaskets, useShopperBasketsMutation } from '@salesforce/commerce-sdk-react';
import * as queryKeyHelpers from '@salesforce/commerce-sdk-react/hooks/ShopperProducts/queryKeyHelpers';
// Chakra
import { Box, useDisclosure, useStyleConfig } from '@salesforce/retail-react-app/app/components/shared/ui';
...
// Others
import { watchOnlineStatus, flatten, mergeMatchedItems, isServer } from '@salesforce/retail-react-app/app/utils/utils';
...
import { EcomApiProvider } from '../../../../app/contexts/ecomAPI/EcomApiContext';
/** CFC End **/
...
const App = (props) => {
  ...
  const is404ForMissingTranslationFile = /\/static\/translations\/compiled\/[^.]+\.json$/.test(location?.pathname);
  ...
  const { data: baskets } = useCustomerBaskets({ parameters: { customerId: customer.customerId } }, { enabled: !!customer.customerId && !isServer });
  ...
  useEffect(() => {
    ...
    if (basket && customer?.isRegistered && customer?.email && customer?.email !== basket?.customerInfo?.email) {
              ...
              <CurrencyProvider currency={currency}>
                <Seo>
                  ...
                  {site.l10n?.supportedLocales.map((locale) => (
                    <link rel="alternate" hrefLang={locale.id.toLowerCase()} href={`${appOrigin}${buildUrl(location.pathname)}`} key={locale.id} />
                  ))}
                  {/* A general locale as fallback. For example: "en" if default locale is "en-GB" */}
                  <link rel="alternate" hrefLang={site.l10n.defaultLocale.slice(0, 2)} href={`${appOrigin}${buildUrl(location.pathname)}`} />
                ...
                <Box id="app" display="flex" flexDirection="column" flex={1}>
                  ...
                  <Box {...styles.headerWrapper}>
                    {!isCheckout ? (
                      <>
                        ...
                        >
                          <HideOnDesktop>
                            <DrawerMenu isOpen={isOpen} onClose={onClose} onLogoClick={onLogoClick} root={categories?.[CAT_MENU_DEFAULT_ROOT_CATEGORY]} />
                          </HideOnDesktop>
                  ...
                  <AddToCartModalProvider>
                    ...
                    >
                      <Box as="main" id="app-main" role="main" display="flex" flexDirection="column" flex="1">
