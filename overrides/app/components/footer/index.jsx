...
import { ecomNavigationFamily } from '../../../../app/contexts/state/atoms';
import { useEcomNavigation } from '../../../../app/contexts/ecomAPI/EcomNavigation';
import { toFSLocale } from '../../../../app/utils/locale';
...
const Footer = ({ ...otherProps }) => {
  ...
  /** CFC Start **/
  const { getLocalizedSeoUrl } = useEcomNavigation();
  ...
  /** CFC Start **/
  const getNewUrl = async (locale, seoId) => {
    return await getLocalizedSeoUrl(seoId, toFSLocale(locale));
  };
  /** CFC End **/
        ...
        <StylesProvider value={styles}>
          ...
          {showLocaleSelector && (
            <Box {...styles.localeSelector}>
              <FormControl data-testid="sf-footer-locale-selector" id="locale_selector" width="auto" {...otherProps}>
                <Select
                  defaultValue={locale}
                  onChange={async ({ target }) => {
                    ...
                    /** CFC Start **/
                    const path = window?.location?.pathname.replace(/\/[a-zA-Z]{2}-[a-zA-Z]{2}/, '');
                    const seoId = navigation?.seoRouteMap?.[`${path}.json`];

                    if (seoId) {
                      //Update URL.
                      window.location = getPathWithLocale(target.value, buildUrl, {
                        disallowParams: ['refine'],
                        location: {
                          pathname: await getNewUrl(target.value, seoId),
                          search: '',
                        },
                      });
                    } else {
