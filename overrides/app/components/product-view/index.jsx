...
import { Box, Button, Fade, Flex, Heading, Skeleton, Text, useTheme, VStack } from '@salesforce/retail-react-app/app/components/shared/ui';
...
import ImageGallery, { Skeleton as ImageGallerySkeleton } from '@salesforce/retail-react-app/app/components/image-gallery';
...
const ProductViewHeader = ({ name, basePrice, discountPrice, currency, category, productType }) => {
  ...
  return (
    <VStack mr={4} spacing={2} align="flex-start" marginBottom={[4, 4, 4, 0, 0]}>
      ...
      <DisplayPrice basePrice={basePrice} discountPrice={discountPrice} currency={currency} isProductASet={isProductASet} />
    </VStack>
  );
};
  ...
  (
    {
      ...
      onVariantSelected = () => {},
      validateOrderability = (variant, quantity, stockLevel) => !isProductLoading && variant?.orderable && quantity > 0 && quantity <= stockLevel,
    },
    ref
  ) => {
    ...
    const { isOpen: isAddToCartModalOpen, onOpen: onAddToCartModalOpen, onClose: onAddToCartModalClose } = useAddToCartModalContext();
      ...
      if (addToCart || updateCart) {
        buttons.push(
          <Button
            ...
            marginBottom={4}
          >
            {updateCart ? buttonText.update : isProductASet ? buttonText.addSetToCart : buttonText.addToCart}
          </Button>
        );
      }
      ...
      if (addToWishlist || updateWishlist) {
        buttons.push(
          <ButtonWithRegistration
            ...
            marginBottom={4}
          >
            {updateWishlist ? buttonText.update : isProductASet ? buttonText.addSetToWishlist : buttonText.addToWishlist}
          </ButtonWithRegistration>
        );
      }
      ...
      <Flex direction={'column'} data-testid="product-view" ref={ref}>
        ...
        <Flex direction={['column', 'column', 'column', 'row']}>
          <Box flex={1} mr={[0, 0, 0, 6, 6]}>
            {product ? (
              <>
                <ImageGallery size={imageSize} imageGroups={product.imageGroups} selectedVariationAttributes={variationParams} lazy={isProductPartOfSet} />
                <HideOnMobile>
          ...
          <VStack align="stretch" spacing={8} flex={1}>
            ...
            {/** CFC Start */}
            <EcomPageSlot key={product?.id} slotName="sup_content">
                    ...
                    return (
                      ...
                      >
                        {values.map(({ href, name, image, value, orderable }) => (
                          <Swatch key={value} href={href} disabled={!orderable} value={value} name={name}>
                            {image ? (
                              <Box
                                height="100%"
                                width="100%"
            ...
            <Box>
              ...
              <Box display={isProductPartOfSet ? 'block' : ['none', 'none', 'none', 'block']}>{renderActionButtons()}</Box>
            </Box>
          </VStack>
        </Flex>
