...
import { Button, Text, Flex, Stack, Link } from '@salesforce/retail-react-app/app/components/shared/ui';
...
const EmptySearchResults = ({ searchQuery, category }) => {
  const intl = useIntl();
  return (
    <Flex data-testid="sf-product-empty-list-page" direction="column" alignItems="center" textAlign="center" paddingTop={28} paddingBottom={28}>
      ...
      {!searchQuery ? (
        ...
        <Fragment>
          ...
          <Stack spacing={16} marginTop={32}>
            <RecommendedProducts
              title={<FormattedMessage defaultMessage="Top Sellers" id="empty_search_results.recommended_products.title.top_sellers" />}
              recommender={EINSTEIN_RECOMMENDERS.EMPTY_SEARCH_RESULTS_TOP_SELLERS}
              mx={{ base: -4, md: -8, lg: 0 }}
            />

            <RecommendedProducts
              title={<FormattedMessage defaultMessage="Most Viewed" id="empty_search_results.recommended_products.title.most_viewed" />}
